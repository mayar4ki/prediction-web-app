...
export const useBetsTable = ({ data }: UseBetsTableProps) => {
    ...
    const columns: ColumnDef<RoundData>[] = [
        ...
        {
            header: `Total volume (${nativeSymbol} / USD)`,
            accessorKey: "totalVolume",
            cell: ({ row }) => {
                const tmp = Number(formatUnits(row.getValue<bigint>("totalVolume"), 18));
...

        {
            ...
            cell: ({ row }) => {
                ...
                return h(
                    "div",
                    { class: "capitalize" },
                    weiShare.toString() + " wei /  " + formatCurrency(convertEthToUsd(tmp))
                );
            },
        },
